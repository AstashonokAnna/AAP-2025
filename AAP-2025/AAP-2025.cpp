...
int wmain(int argc, wchar_t* argv[])
{
	...
	try
	{
		...
		*logfile.stream << "\tÐàçáîð âûïîëíåí çà " << ((float)search_time) / CLOCKS_PER_SEC << " ñåêóíä." << std::endl;
		*logfile.stream << std::setfill('-') << std::setw(90) << '-' << std::endl;

		std::cout << "\tÐàçáîð âûïîëíåí çà " << ((float)search_time) / CLOCKS_PER_SEC << " ñåêóíä." << std::endl;
		...
		//LT::WriteLexTable(lex.lextable);
		std::cout << "\t\tÏÎËÜÑÊÀß ÍÎÒÀÖÈß ÂÛÏÎËÍÅÍÀ" << std::endl;
		std::cout << std::setfill('-') << std::setw(90) << '-' << std::endl;
		if (parm.tokens)
			LT::WriteTokens(lex.lextable);

		*logfile.stream << "\t\tÏÎËÜÑÊÀß ÍÎÒÀÖÈß ÂÛÏÎËÍÅÍÀ" << std::endl;
		*logfile.stream << std::setfill('-') << std::setw(90) << '-' << std::endl;
		Log::WriteTokensLog(lex.lextable, logfile);
